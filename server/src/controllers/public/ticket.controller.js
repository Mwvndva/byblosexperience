...
/**
 * Verify Paystack payment and purchase tickets
 */
export const verifyPaystackAndPurchase = async (req, res, next) => {
  const { reference, ...ticketData } = req.body;
  if (!reference) {
    return next(new AppError('Missing Paystack payment reference', 400));
  }
  try {
    ...
    // Call the existing purchaseTickets logic
    req.body = ticketData; // Remove reference, keep ticket info
    return purchaseTickets(req, res, next);
  } catch (error) {
    return next(new AppError('Failed to verify Paystack payment', 500));
  }
};
