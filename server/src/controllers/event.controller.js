...
export const createEvent = async (req, res) => {
  ...
  try {
    ...
    try {
      console.log('Processing ticket types. Raw ticketTypes:', JSON.stringify(ticketTypes, null, 2));
      console.log('ticket_quantity:', ticket_quantity, 'ticket_price:', ticket_price);
      // If no ticket types provided but we have direct ticket info, create a default ticket type
      if ((!ticketTypes || ticketTypes.length === 0) && ticket_quantity !== undefined && ticket_price !== undefined) {
        const price = typeof ticket_price === 'string' ? parseFloat(ticket_price) : Number(ticket_price);
        const quantity = typeof ticket_quantity === 'string' ? parseInt(ticket_quantity, 10) : Math.max(1, Math.floor(Number(ticket_quantity)));
